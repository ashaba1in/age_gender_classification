...
class AgeHead(nn.Module):
    def __init__(self, in_features: int):
        ...
        self.fc2 = nn.Linear(in_features, config['num_classes_age'])
        self.act = nn.ReLU()
        self.dr1 = nn.Dropout(p=config['dropout1'])
...
class GenderHead(nn.Module):
    def __init__(self, in_features: int):
        ...
        self.fc2 = nn.Linear(in_features // 2, config['num_classes_age'])
        self.act = nn.ReLU()
        self.dr1 = nn.Dropout(p=config['dropout1'])
        self.dr2 = nn.Dropout(p=config['dropout2'])
...
class Model(nn.Module):
    def __init__(self, load_pretrained: bool = False, path_pretrained: str = None):
        ...
        if load_pretrained:
            self._load_pretrained(path_pretrained)

    def forward(self, x):
        return self.head1(self.backbone1(x)), self.head2(self.backbone2(x))
