...
class LossLayer(nn.Module):
    ...
    def forward(self, classifications, bbox_regressions, ldm_regressions, anchors, annotations):
        ...
        for j in range(batch_size):
            ...
            print(anchors[0, :, :].shape, bbox_annotation[:, :4].shape)
            print(IoU.shape)
            ...
            print(negative_indices.shape, targets.shape)
        ...
        return torch.stack(classification_losses), torch.stack(bbox_regression_losses), torch.stack(
            ldm_regression_losses)
